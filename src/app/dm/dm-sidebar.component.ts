...
@Component({
  ...
  template: `
    <div
      class="fixed z-10 flex h-12 w-60 items-center justify-center bg-zinc-850 p-2 shadow"
    >
      <button
        class="flex h-7 w-56 items-center rounded-sm bg-zinc-800 p-2 text-sm font-normal shadow-inner"
      >
    ...
    <div class="mt-12 flex flex-col gap-px p-2">
      ...
      <label
        class="flex flex-row justify-between p-3 pb-1 pt-4 text-xs font-medium hover:text-zinc-300"
      >
        MENSAJES DIRECTOS
      ...
      >
        ...
        <button
          class="scale-0 text-zinc-400 hover:text-zinc-200 group-hover:scale-100 group-active:text-zinc-200"
        >
  ...
  `,
  styles: [
    ':host { @apply overflow-auto flex-shrink-0 w-60 scrollbar-hide relative text-zinc-400 font-medium text-base bg-zinc-850 }'
  ],
  imports: [
    CommonModule,
    UserStatusAreaComponent,
    RouterModule,
    UserAvatarComponent
  ]
})
